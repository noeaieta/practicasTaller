...
const char* RUTA_PLATAFORMAS = "archivoPlataformas.txt";
...
nivel_t* parsearLineaEscenario(char* linea, const char* separador){
	...
	while (sep1 != NULL && sep2 != NULL && sep3 != NULL){
		...
		plataforma_asignarID(plat, atoi(sep3));
...
void calcularEImprimir(barril_t** barriles, unsigned int cantidad, escenario_t* escenario){
	...
	barril_ordenarPorDistancia(barriles, cantidad);
...
plataforma_t* buscarPlataforma(escenario_t* e, int id){
	unsigned int i, j;
	for (i = 0; i < e->cantidad; i++)
		for (j = 0; j < e->niveles[i]->cantidad; j++)
			if (e->niveles[i]->plataformas[j]->ID == id)
				return e->niveles[i]->plataformas[j];
	return NULL;
	
}

void cargarObstaculos(escenario_t* es){
	FILE* archivo = fopen(RUTA_PLATAFORMAS, "r");
	printf("todo bien 1\n");
	unsigned int cant;
	char** lineas = obtenerLineas(archivo, &cant);
	printf("todo bien 2\n");
	unsigned int i;
	for (i = 0; i < cant; i++){
		char* sep = strtok(lineas[i], SEPARADOR);
		char* sep2 = strtok(NULL, SEPARADOR);
		printf("todo bien 3\n");
		plataforma_t* plat = buscarPlataforma (es, atoi(sep));
		if (plat)
			plataforma_agregarObstaculo(plat, strtof(sep2, NULL));		
		free(lineas[i]);
		printf("todo bien 4\n");
	}
	printf("todo bien 5\n");
