	...
	for (i = 0; i < es->cantidad; i++){
		...
		for (j = 0; j < es->niveles[i]->cantidad && !salir; j++){
			...
			if (orden == DERECHA){
				x = plataformas[j]->xf;
				distAct = x - aux->x;
				float xobs = plataforma_tiene_obstaculo(plataformas[j], aux->x, x, INICIO);
				if (xobs != NOHAY){
					x = plataformas[j]->xi;
					float xobs_2 = plataforma_tiene_obstaculo(plataformas[j], x,aux->x, FINAL);
					if (xobs_2 != NOHAY){
						*inf = true;
					}else{
						distAct = xobs - aux->x;
				...
				x = plataformas[j]->xi;
				distAct =  aux->x - x;
				float xobs = plataforma_tiene_obstaculo(plataformas[j], x ,aux->x, FINAL);
				if (xobs != NOHAY){
					x = plataformas[j]->xf;
					float xobs_2 = plataforma_tiene_obstaculo(plataformas[j], x,aux->x, INICIO);
					if (xobs_2 != NOHAY){
						*inf = true;
					}else{
						distAct = aux->x - xobs;
